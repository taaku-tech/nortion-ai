...
Vercel Cron は毎日 23:00 UTC（JST 翌 08:00）に `/api/cron/sync-notion`、10分後の 23:10 UTC に `/api/cron/process-pages` を呼び出す。JST 土日の実行はコードレベルでスキップされる。
...
DBに保存済みの本文キャッシュを使い、Gemini トピック抽出と embedding 生成を行う。Notion API は呼ばない。Vercel Function `maxDuration = 60`（秒）。

**Cron スケジュール:** `10 23 * * *`（UTC 23:10 ＝ JST 翌 08:10）
