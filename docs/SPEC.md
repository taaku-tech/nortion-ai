...
Vercel Cron が平日 23:00 UTC（JST 翌 08:00、UTC 日〜木）に `/api/cron/extract` を呼び出す。JST 土日の手動実行はコードレベルでもスキップされる。
...
**Cron スケジュール:** `0 23 * * 0-4`（UTC 日〜木 23:00 ＝ JST 月〜金 08:00）
...
> Vercel Cron 自体は UTC 日〜木のみ実行のため、通常はこのチェックが発動しない。手動で土日に叩いた場合の安全装置として機能する。
...
| ログキー | 出力タイミング | 主な内容 |
|---------|------------|---------|
| `[cron:extract] start` | cron開始直後 | startedAt, weekdayJst, isWeekendJst |
| `[cron:extract] skip` | 土日スキップ時のみ | reason: weekend_jst |
| `[cron:extract] zombie reset` | ゾンビリセット直後 | count（件数）, pages（最大5件の pageId/title） |
| `[cron:extract] targets selected` | targets取得直後 | count, pending数, doneNoEmbedding数 |
| `[cron:extract] page start` | 各ページ処理開始 | pageId, title, previousStatus, retryCount |
| `[cron:extract] page done` | 各ページ処理完了 | pageId, result（done/skipped/error/permanent_error）, errorType, errorMsg（200文字truncate） |
| `[cron:extract] end` | cron終了直前 | done/error/skipped/embeddingGenerated/zombieReset/remaining |
...
**non-retryable の判定:** `NotionApiError.isNonRetryable === true`（`geminiClient.isNonRetryable(err)` で確認）。`GEMINI_PARSE` / `GEMINI_TIMEOUT` は non-retryable ではなく `error` 扱い（retryable）。
...
- ログ: `[cron:extract] page done { pageId, result: 'skipped' }`
